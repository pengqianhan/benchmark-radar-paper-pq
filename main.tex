...
Figure~\ref{fig:taxonomy-trends} focuses on the \TaxonomyWindowDated{} records with documented benchmark release or first-introduction dates from \TaxonomyWindowFirst{}, using a \TaxonomySharePeriod{} axis to show recent trends. We match the \FindingsMissingRelease{} originally undated source records against \ReportNumber{\TaxonomyLibraryReviewedDates} Library records with reviewed date evidence, adding \TaxonomyLibraryDates{} applicable release or introduction dates. A second pass adds \TaxonomyLegacyDates{} dates from previously passed version-specific reviews, joined by exact source-record keys and read from their latest verified release-date field. The combined \TaxonomySupplementedDates{} annotations preserve the frozen census and discovery cutoff. Matches require source-specific identity or an explicit identity review. Parent-dataset dates and later result disclosures are withheld unless the evidence establishes introduction of the exact named component; the audit retains event scope, day or month precision, and every unmatched record. Because historical coverage is incomplete, the \TaxonomyPreWindowDated{} records dated before 2023 are omitted from Panel~A while remaining in the full census and audit. The \TaxonomyTrendUndated{} records without a release date retain a separate column. A period's share is read only where the evidence supports one, which takes more than a sufficient count: the OpenCompass Hub crawl stops at the discovery cutoff, so the truncated final periods are dominated by LLM Stats and model reports and their shares would measure the change of catalog rather than a change in the field. We therefore also require a period's source mix to stay close to the pooled mix. Within the eligible periods, reweighting to a common source composition changes the agentic share by up to \TaxonomyMixAdjustment{} percentage points. This sensitivity remains material: the curves describe dated records in the collected catalog and do not estimate field-wide benchmark release rates.

\begin{center}
\begin{minipage}{\textwidth}
\centering
\includegraphics[width=\textwidth]{taxonomy-trends.pdf}
\captionof{figure}{Recent release or first-introduction trends with \TaxonomySupplementedDates{} date supplements (\TaxonomyLibraryDates{} Library, \TaxonomyLegacyDates{} prior verified). Counts are source records, not independent benchmark releases. Panel~A shows \TaxonomyWindowDated{} dated records from \TaxonomyWindowFirst{} and \TaxonomyTrendUndated{} undated records on the right with their own scale. Legend counts cover these \ReportNumber{\TaxonomyPanelAShown} shown records. The \TaxonomyPreWindowDated{} earlier records are omitted for incomplete historical coverage but retained in the census. The last \TaxonomySharePeriod{} stops at the discovery cutoff. Incomplete source coverage means trailing bars should not be interpreted as a decline. Panel~B shows \TaxonomySharePeriod{}s with at least \TaxonomyMinPeriodRecords{} dated records and a source mix close to the pooled one: \TaxonomyReportedPeriods{}. The \TaxonomyExcludedFromShares{} dated records outside these periods remain in the full audit. Among Level~1 classes and facets with at least \TaxonomySeriesMinRecords{} records in the reported periods, the \TaxonomySeriesDrawn{} selected series have the largest pooled share movements between the earlier and later halves, skipping series more than \TaxonomySeriesMaxContainment{} contained in one already selected. Legends report movement in percentage points; the trends evidence file records every candidate and selection decision. Facets overlap Level~1 classes, so Panel~B's shares do not sum to 100\%.}
...
The frozen catalog records benchmark release dates for \CensusReleaseKnown{} records, leaving \FindingsMissingRelease{} without a release date. Figure~\ref{fig:taxonomy-trends} separately adds \TaxonomySupplementedDates{} retrospective date annotations to these existing records; this does not alter the released census. Across score observations, \ReportNumber{\FindingsModelDateScores} entries carry model-announcement dates and \FindingsDocumentDateScores{} carry document-publication dates. Neither date basis directly establishes an evaluation date. Researchers must first establish dates and comparable evaluation settings for the relevant source records.
