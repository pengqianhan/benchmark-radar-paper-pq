...
Figure~\ref{fig:taxonomy-trends} focuses on benchmark release or first-introduction dates from \TaxonomyWindowFirst{}, grouped by \TaxonomySharePeriod{} and Level~1 domain. The \TaxonomyPreWindowDated{} earlier records remain in the catalog but are omitted because historical coverage is incomplete. Over \TaxonomyReportedPeriods{}, agentic evaluation exhibits the largest share increase among the displayed series, whereas knowledge and factuality, image-based evaluation, and mathematical reasoning have lower pooled shares in the later periods. These patterns describe the collected catalog. Incomplete and varying source coverage precludes interpreting them as field-wide release rates or the smaller trailing bars as declining benchmark activity.

\begin{center}
\begin{minipage}{\textwidth}
\centering
\includegraphics[width=\textwidth]{taxonomy-trends.pdf}
\captionof{figure}{Temporal distribution and changing composition of benchmark records. Counts refer to source records, not independent benchmark releases. (A) Release or first-introduction dates by \TaxonomySharePeriod{} and Level~1 domain from \TaxonomyWindowFirst{}: \TaxonomyWindowDated{} dated records and \TaxonomyTrendUndated{} undated records, totaling \ReportNumber{\TaxonomyPanelAShown} records in the legend. Undated records use the same vertical scale as the dated records; \TaxonomyPreWindowDated{} earlier records are outside the displayed window. The final period ends at the \datacutoff{} cutoff. (B) Shares of \TaxonomySeriesDrawn{} selected domains and facets among all dated records in each period during \TaxonomyReportedPeriods{}. These periods contain at least \TaxonomyMinPeriodRecords{} dated records and have source compositions close to the pooled catalog. The selected series represent large changes in pooled share. Legend changes, in percentage points, compare the earlier and later halves of the displayed periods, excluding the middle period. Facets overlap domains, so the plotted shares do not sum to 100\%.}
...
For the temporal analysis in Figure~\ref{fig:taxonomy-trends}, \ReportNumber{\TaxonomyTrendDated} of the \ReportNumber{\CensusRecords} source records have documented benchmark release or first-introduction dates, while \TaxonomyTrendUndated{} remain undated. Across score observations, \ReportNumber{\FindingsModelDateScores} entries carry model-announcement dates and \FindingsDocumentDateScores{} carry document-publication dates. Neither date basis directly establishes an evaluation date. Researchers must first establish dates and comparable evaluation settings for the relevant source records.
